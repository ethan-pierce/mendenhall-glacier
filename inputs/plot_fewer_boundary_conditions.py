...
im0 = ax[0, 0].imshow(plot0, cmap = 'Blues_r')
ax[0, 0].set_title('Bedrock elevation (m)')
...
im1 = ax[0, 1].imshow(plot1, cmap = 'Blues_r')
...
im2 = ax[0, 2].imshow(plot2, cmap = 'RdBu_r', vmax = 400)
ax[0, 2].set_title('Observed surface velocity (m a$^{-1}$)')
plt.colorbar(im2, ax = ax[0, 2], fraction = 0.0543, pad = 0.04, extend = 'max')

plot3 = np.where(plot1 > 0.5, geology['slidingco'][:], np.nan)
im3 = ax[1, 0].imshow(plot3, cmap = 'RdBu_r')
...
im4 = ax[1, 1].imshow(plot4, cmap = 'RdBu_r', vmax = 250)
ax[1, 1].set_title('Modeled sliding velocity (m a$^{-1}$)')
plt.colorbar(im4, ax = ax[1, 1], fraction = 0.0543, pad = 0.04, extend = 'max')

plot5 = np.where(plot1 > 0.5, np.sqrt(glacier['uvelsurf'][0]**2 + glacier['vvelsurf'][0]**2), np.nan)
im5 = ax[1, 2].imshow(plot5, cmap = 'RdBu_r', vmax = 400)
ax[1, 2].set_title('Modeled surface velocity m a$^{-1}$)')
plt.colorbar(im5, ax = ax[1, 2], fraction = 0.0543, pad = 0.04, extend = 'max')
